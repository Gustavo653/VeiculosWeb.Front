...
import { ColorService } from 'src/app/demo/service/color.service';
...
export class ColorComponent implements OnInit {
    ...
    save() {
        if (this.validateData()) {
            ...
            this.loading = true;
            if (data.id) {
                this.colorService.updateColor(data.id, data).subscribe(() => {
                ...
                this.colorService.createColor(data).subscribe(() => {
                    this.messageService.add(MessageServiceSuccess);
                    this.fetchData();
                }, () => {
                    this.loading = false;
    ...
    deleteRegistry(registry: any) {
        this.confirmationService.confirm({
            ...
            accept: () => {
                this.loading = true;
                this.colorService.deleteColor(registry.id).subscribe(() => {
                    this.messageService.add(MessageServiceSuccess);
                    this.fetchData();
                }, () => {
                    this.loading = false;
    ...
    fetchData() {
        this.colorService.getColors().subscribe((x) => {
            this.data = x.object;
            this.loading = false;
        });
    }
